...
def _train_fold(
    train_cite_X, train_cite_y, test_cite_X, train_idx, valid_idx, params, num_boost_round, early_stopping_rounds
):
    """Train one LightGBM model per target on a single fold.
    ...
    valid_preds = np.zeros((len(valid_idx), n_targets))
    test_preds = np.zeros((test_cite_X.shape[0], n_targets))
    best_iterations = np.zeros(n_targets, dtype=int)

    for i in range(n_targets):
        dtrain.set_label(train_cite_y[train_idx, i])
        dval.set_label(train_cite_y[valid_idx, i])
...
def train_lightgbm_kfold(
    ...
    n_jobs : int = 1
        Number of worker processes. Every process handles one fold and a chunk of the targets at a time, so its
        memory footprint is one binned fold. Training several targets in parallel with a few threads each is much
        more efficient than one target with many threads, because a single LightGBM model on tens of thousands of
        cells parallelizes poorly beyond a handful of threads. `-1` uses all available CPUs.
    ...
    params = {**params, "num_threads": max(1, total_threads // n_jobs)}
    ...
    logger.info(
        ...
        f"with {n_jobs} process(es) x {params['num_threads']} thread(s)"
    )
    if n_jobs == 1:
        results = (_run(fold, chunk) for fold, chunk in tasks)
    else:
    ...
    for n_done, (fold, chunk, (valid_preds, fold_test_preds, fold_best_iterations)) in enumerate(results, start=1):
        ...
        logger.info(
            f"{n_done}/{len(tasks)} fold-chunk tasks done in {elapsed / 60:.1f} min "
            f"(fold {fold}, {len(chunk)} targets, best iterations {int(fold_best_iterations.min())}-{int(fold_best_iterations.max())})"
        )
