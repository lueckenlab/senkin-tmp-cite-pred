...
from senkin_tmp_cite_pred.preprocess import to_dense

logger = logging.getLogger(__name__)


def zscore(x):
    """Z-score every row (cell) of a matrix: subtract the row mean and divide by the row standard deviation.

    Rows with zero standard deviation (constant rows) are set to 0 instead of NaN.
    """
    x = to_dense(x)
...
def prepare_nn_inputs(*blocks, dtype=np.float32):
    """Build the input matrix of the neural networks from feature blocks.

    As in the original solution (https://www.kaggle.com/code/senkin13/2nd-place-gru-cite), every block
    (e.g. CLR-TSVD components, selected raw genes, or TSVD-reduced LightGBM predictions) is z-scored
    per cell separately before the blocks are concatenated. This puts blocks of very different scales
    (raw counts, SVD components, predictions) on a comparable footing.

    Parameters
    ----------
    *blocks : array-like
        Matrices of shape (n_cells, n_features_i), dense or sparse, with the same cells in the same order.
    ...
    -------
    np.ndarray
        Matrix of shape (n_cells, sum(n_features_i)).
    """
    return np.concatenate([zscore(block) for block in blocks], axis=1).astype(dtype)
...
def nn_kfold(
    ...
    """Train a network once per fold. Returns out-of-fold predictions for the training cells and
    fold-averaged predictions for the test cells. The best (by validation loss) weights of every fold
    are saved to `models_dir`."""
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_cell_ids)):
        ...
        filepath = os.path.join(models_dir, model_name + "_" + str(n_fold) + ".weights.h5")
        es = tf.keras.callbacks.EarlyStopping(patience=10, mode="min", verbose=1)
        checkpoint = tf.keras.callbacks.ModelCheckpoint(
            monitor="val_loss", filepath=filepath, save_best_only=True, save_weights_only=True, mode="min"
        )
        ...
        train_preds[valid_idx] = model.predict(valid_x, batch_size=BATCH_SIZE, verbose=1)

        oof_corr = correlation_score(valid_y, train_preds[valid_idx])
        cv_corr.append(oof_corr)
        logger.info(f"Fold correlation scores: {cv_corr}")

        test_preds += model.predict(test_cite_X, batch_size=BATCH_SIZE, verbose=1) / folds.n_splits

        del model
        gc.collect()
        tf.keras.backend.clear_session()
    cv = correlation_score(train_cite_y, train_preds)
    logger.info(f"Overall: {cv}")
...
def cite_cos_sim_model(len_num, n_targets=140):
    ...
    lr = 0.001
    # The original passed epsilon=None, which in Keras 2 meant the default value of 1e-7
...
def train_nn_models(
    train_cell_ids, train_cite_X, train_cite_y, test_cell_ids, test_cite_X, folds, EPOCHS=100, models_dir="models"
):
    ...
    Parameters
    ----------
    train_cell_ids : array-like
        Cell IDs for training
    train_cite_X : np.ndarray
        Array of training features. Build it with `prepare_nn_inputs` so that every feature block is
        z-scored per cell, as in the original solution.
    train_cite_y : np.ndarray
        Array of training targets (normalized protein expression)
    test_cell_ids : array-like
        Cell IDs for testing
    test_cite_X : np.ndarray
        Array of testing features, built with `prepare_nn_inputs`
    folds : sklearn.model_selection.KFold
        KFold object for cross-validation
    EPOCHS : int
        Number of epochs to train the models
    models_dir : str
        Directory to save the best weights of every fold to

    Returns
    -------
    train_preds : np.ndarray
        Array of (out-of-fold) training predictions, z-scored per cell
    test_preds : np.ndarray
        Array of testing predictions, z-scored per cell
    """
    train_cell_ids = np.asarray(train_cell_ids)
    test_cell_ids = np.asarray(test_cell_ids)
    train_cite_X = to_dense(train_cite_X, dtype=np.float32)
    test_cite_X = to_dense(test_cite_X, dtype=np.float32)
    train_cite_y = to_dense(train_cite_y, dtype=np.float32)
